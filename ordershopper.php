...
<body class="homeshopper">
  ...
  <section>
    <header class="myorder">
      ...
      <?php
      $sql = "select * from orders JOIN product ON product.ProductID = orders.ProductID JOIN seller ON product.SellerID = seller.SellerID where orders.CustomerID =$customerid";
      $result = $conn->query($sql);
      if($result->num_rows > 0) {
        while($row=mysqli_fetch_array($result)){
        echo "
        <div class='order'>
          <a href='orderdetailsshopper.php?OrderID=".$row['OrderID']."'>
          <div class='productimage'>
            <img class='img' src='Products/".$row['ProductImage']."'>
          </div>
          <div class='orderdetails'>
            <div class='productname'><h3>".$row['ProductName']."</h3></div>
            <div class='matricno'><h3>RM".$row['TotalPrice']."</h3></div>
